...
## 简介
FishWorker 是一款面向个人效能的桌面应用，围绕“回顾—习惯—清单—时间—使命—写作”六大能力构建。本概览文档聚焦以下模块：
- 每日回顾系统：结构化复盘与自动保存，沉淀成长轨迹
- 习惯追踪：可视化打卡、统计与详情面板，驱动行为改变
...
HAB["习惯追踪<br/>HabitPanel / HabitList / HabitCard"]
...
## 核心组件
本节概述六大模块的职责边界与关键交互点，便于快速定位深入文档入口。
...
- 习惯追踪
  - 职责：创建习惯、标记完成、查看连续天数与趋势
  - 关键文件：[HabitPanel.tsx](file://src/features/habits/HabitPanel.tsx)、[HabitList.tsx](file://src/features/habits/components/HabitList.tsx)、[HabitCard.tsx](file://src/features/habits/components/HabitCard.tsx)、[HabitDetailSidebar.tsx](file://src/features/habits/components/HabitDetailSidebar.tsx)、[HabitEditModal.tsx](file://src/features/habits/components/HabitEditModal.tsx)
  - 关联：可与使命的目标对齐，形成“目标—习惯—日常”闭环
...
### 习惯追踪
- 功能描述：创建习惯、每日打卡、查看连续天数与详情侧边栏
- 使用场景：
  - 用户故事：设定“每天阅读 30 分钟”，完成后一键打勾，连续天数可视化激励坚持
  - 实际案例：将“运动”习惯与“健康”目标绑定，定期复盘完成率
- 价值说明：用可见反馈强化正向行为，降低意志力消耗
- 关键流程（序列图）：
```mermaid
sequenceDiagram
participant U as "用户"
participant HP as "HabitPanel"
participant HL as "HabitList"
participant HC as "HabitCard"
participant HDS as "HabitDetailSidebar"
participant HM as "HabitEditModal"
participant ST as "习惯Store(外部)"
participant SVC as "习惯Service(外部)"
participant T as "Tauri 调用"
participant API as "habit.rs(占位)"
participant D as "db.rs"
U->>HP : 打开习惯面板
HP->>HL : 渲染列表
U->>HC : 点击完成/编辑
...
- 懒加载与分页：习惯与清单数据量大时，按需加载与分页可提升首屏速度
- 编辑器增量同步：TipTap 内容变更尽量增量同步，避免全量覆盖
- 缓存策略：对只读统计类数据（如习惯连续天数）增加短期缓存
...
- 常见问题
  ...
  - 习惯打卡无反馈：验证 HabitCard 事件冒泡与 HabitDetailSidebar 状态更新
...
## 结论
FishWorker 以清晰的特性划分与分层架构，将“回顾—习惯—清单—时间—使命—写作”串联成完整的个人效能闭环。TipTap 编辑器作为通用能力提升了内容创作效率；Tauri + Rust 后端保障数据安全与性能。建议在后续迭代中持续完善数据模型与跨模块联动，进一步提升用户体验与可维护性。
...
- 进一步阅读导航
  ...
  - 习惯追踪：[HabitPanel.tsx](file://src/features/habits/HabitPanel.tsx)、[HabitCard.tsx](file://src/features/habits/components/HabitCard.tsx)
