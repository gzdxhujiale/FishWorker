...
- tiptap-node：具体节点实现（图片上传节点、分隔线节点等）
...
subgraph "UI 层"
E["image-upload-button.tsx"]
F["use-image-upload.ts"]
end
subgraph "工具层"
G["tiptap-utils.ts"]
end
...
## 结论
通过统一的扩展与节点模式，本项目实现了背景色、图片上传与分隔线三类自定义节点的可插拔能力。UI 层通过命令与 Hook 与节点层解耦，配合工具函数提升复用性。建议在后续迭代中完善错误边界、可访问性与国际化支持，并持续优化大图与多文件上传体验。
