...
## 更新摘要
**所做更改**   
- 移除了原桌面应用根工程中的编码规范文档引用
- 更新了项目结构说明以反映当前 FishWorker 项目的实际组织方式
- 强化了 TypeScript、React 和 Rust 三层技术栈的编码规范定义
- 完善了 ESLint 和 Prettier 配置说明，提供具体的工具链集成指导
...
## 简介
本规范面向 FishWorker 项目的 TypeScript、React 与 Rust 三层技术栈，目标是统一编码风格、类型约定、组件结构与文档注释标准，并明确 ESLint/Prettier 及静态检查工具的使用方式。同时给出 Tauri 后端 Rust 模块的命名与错误处理约定，确保前后端协作一致、可维护性与可读性提升。

FishWorker 是一个基于 Tauri 构建的桌面应用，采用现代化的前端技术栈（React + TypeScript + Vite）与高性能的 Rust 后端服务，通过功能域驱动的架构模式组织代码。
...
- 模块与命名：模块按领域划分，文件名与模块名一致；函数使用 snake_case。
...
## 结论
本规围绕类型安全、组件一致性、Rust 模块清晰性与错误处理标准化展开，结合现有仓库结构给出落地建议。建议在团队内推广并配合自动化检查工具持续保障质量。
