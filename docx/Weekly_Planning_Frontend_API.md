...
- **添加角色**
  ...
  * 创建一个新角色并随机赋予主题色。数据写入本地并触发低频同步 (`triggerRoleSync`)。
- **更新角色**
  `updateRole(roleId: string, updates: Partial<Role>, isHighFreq?: boolean): void`
  * 部分修改指定角色属性（如重命名、更换色值）。支持配置是否高频触发防抖同步。
- **删除角色**
  `deleteRole(roleId: string): void`
  * 删除指定角色。系统异步调用 `timeManagementApi.deleteRole` 完成云端清理。
...
* **输入**: `{ role: Role }`
* **说明**: 向数据库插入角色。若 ID 已存在，则更新角色名称和颜色。
