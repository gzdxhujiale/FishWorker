...
### 3.2 包含在本次范围内
- **功能点 1：目标池（左侧边栏）**
  - 移除周计划中创建新角色和删除角色的前端交互与能力。
  - **角色联动（人生罗盘）**：周计划中的角色列表完全读取自「人生罗盘」中添加和管理的角色，实现数据源的统一与联动。当在人生罗盘中新增、修改或删除角色时，周计划的角色列表及分配的任务自动同步更新。
...
| ID | 需求描述 | 优先级 | 备注 |
|----|------------|---|-------|
| FR1 | 角色列表与人生罗盘联动 | P0 | 从人生罗盘获取角色数据，不支持在周计划独立增删 |
...
- **左侧边栏 (Width: 240px)**：展示联动自人生罗盘的角色卡片列表。不包含角色新增和删除的前端交互。
